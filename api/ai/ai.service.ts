...
function mockGenerate(prompt: string, detailLevel: string): string {
    const base = `Study Plan: Mastering ${prompt}\n\n`;
    if (detailLevel === "brief") return base + "1. Introduction\n2. Key Concepts\n3. Practice";
...
export async function getRecommendations(userId: string): Promise<Plan[]> {
    const db = getDB();
    const userPlans = await db.collection<Plan>("plans").find({ userId }).toArray();
...
export async function generateProfileInsights(userId: string): Promise<string> {
    const db = getDB();
    const userPlans = await db.collection<Plan>("plans").find({ userId }).toArray();
...
export async function generateQuiz(topics: string[]): Promise<string> {
